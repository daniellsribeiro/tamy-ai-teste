...
export class Migration20250811040625 extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      `create table "orders" ("id" serial primary key, "total" numeric(10,2) not null, "payment_method" varchar(255) not null, "status" varchar(255) not null, "created_at" timestamptz not null, "updated_at" timestamptz not null);`,
    );

    this.addSql(
      `create table "order_items" ("id" serial primary key, "order_id" int not null, "product_id" int not null, "quantity" int not null, "unit_price" numeric(10,2) not null, "line_total" numeric(10,2) not null, "created_at" timestamptz not null);`,
    );
